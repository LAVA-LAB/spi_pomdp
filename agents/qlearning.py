...
class QLearning(RandomAgent):
    ...
    def export_fsc(self, beta=None):
        fsc = self.new_fsc()
        for m in range(fsc.nM):
            if beta is None:
                fsc.action_distribution[m] = self.greedy_policy(m)
            else:
                fsc.action_distribution[m] = self.softmax_policy(beta, m)
        return fsc

    def new_fsc(self):
        return FiniteObservationHistoryFSC.make_uniform_fsc(self.num_observations, self.num_actions, k=0)

    def greedy_policy(self, memory):
        policy = np.zeros((self.num_observations, self.num_actions))
        for o in range(self.num_observations):
            policy[o] = self._greedy_policy(o, memory)
        return policy

    def _greedy_policy(self, o, _):
        policy_o = np.zeros(self.num_actions)
        policy_o[self._greedy_action(o)] = 1.0
        return policy_o

    def softmax_policy(self, beta=0.2, memory=None):
        assert beta > 0
        policy = np.zeros((self.num_observations, self.num_actions))
        for o in range(self.num_observations):
            policy[o] = self._softmax_policy(o, beta, memory)
        return policy

    def _softmax_policy(self, o, beta, _):
